...
#   mutate(year_end = case_when(
#     !is.na(date_end) ~ str_sub(date_end, 1, 4),
#     is.na(date_end) ~ as.character(NA)
#   ),
#   month_end = case_when(
#     #date_end == "Present" ~ "",
...
<h1 class="title2">Ethan Hurwitz, Ph.D.</h1>
...
```{r academic_positions}
cv_entries %>% 
  filter(type == 'academic_positions' & is.na(exclude)) %>%
...
cv_entries %>% 
  ...
  mutate(what = glue("<span class='cv-entry-title'><b>{what}</b></span>, {where}<br>", 
                      "{additional_info}<br>",
                      "<span class='thesis-title'>Dissertation title:  <i>{additional_info2}</i></span>")) %>%
  mutate(what = gsub("NA", "", what)) %>%
  mutate(what = gsub("<br><br>", "<br>", what)) %>%
  mutate(what = ifelse(!grepl("Ph.D", what), 
                       gsub("Dissertation title: ", "", what),
                       what)) %>%
  #select(year, what) %>% 
  select(dateFormatted, what) %>%
  kable()
...
cv_entries %>% 
  ...
  select(year, what) %>% 
  manual_page_break_after_row("what", "Cognitive") %>%
  kable()
```
...
publications %>%
  ...
  knitr::kable() %>%
  gsub("1990", "Under Review", .) %>%
  gsub(" </span><span class='publication-venue' style='font-style: italic;'>, </span>, <span class='publication-venue'>. </span><span class='publication-doi'>. </span>", "", .)
```
...
cv_entries %>% 
  ...
  #select(year, number, citation) %>%
  select(year, citation) %>%
  #manual_page_break_after_row("citation", "An overview of psilocybin") %>%
  knitr::kable()
...
cv_entries %>% 
  ...
  mutate(where = glue("<span class='cv-entry-title'>{where}</span>"),
         year = ifelse(is.na(year), glue(""), year)) %>%
  select(year, where) %>%
  #manual_page_break_after_row("where", "John Templeton") %>%
  kable()
```
